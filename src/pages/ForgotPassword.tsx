...
import { ArrowRight, ArrowLeft, Eye, EyeOff, User, Lock, ShieldCheck } from 'lucide-react'
import { motion } from 'framer-motion'
import logo from '../assets/logo.png'
import { useI18n } from '../i18n'

type Step = 'nationalId' | 'newPassword' | 'otp'
...
export default function ForgotPassword() {
  ...
  function handleNationalIdSubmit() {
  ...
  function handlePasswordSubmit() {
    if (!newPassword || newPassword.length < 8) {
      setError(t('forgotPassword.errMinChars' as any))
      return
    }
    if (newPassword !== confirmPassword) {
      setError(t('forgotPassword.errMismatch' as any))
      return
    }
    setLoading(true)
    setError('')
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div className="register-card" style={{
          ...
          borderRadius: 20, padding: 40, boxShadow: '0 20px 60px rgba(0,0,0,0.25)',
        }}>

          {/* Step 1: National ID */}
          {step === 'nationalId' && (
            <motion.div key="nationalId" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
              <button
              ...
              <div style={{ textAlign: 'center', marginBottom: 28 }}>
                ...
                <h2 style={{ fontSize: 22, fontWeight: 700, color: '#111', marginBottom: 6 }}>
                  {t('forgotPassword.title' as any)}
                </h2>
                <p style={{ fontSize: 14, color: '#6B7280', lineHeight: 1.6 }}>
                  {t('forgotPassword.enterNationalId' as any)}
                </p>
              </div>
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                >
                  ...
                  <input
                    ...
                    onKeyDown={(e) => e.key === 'Enter' && handleNationalIdSubmit()}
              ...
              {error && <p style={{ fontSize: 13, color: '#EF4444', textAlign: 'center', marginBottom: 16 }}>{error}</p>}

              <motion.button
                whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }}
                onClick={handleNationalIdSubmit} disabled={loading}
                style={{
                  width: '100%', height: 48, background: loading ? '#B8E67A' : '#7CFF01',
                  color: '#0F172A', fontWeight: 600, fontSize: 15, borderRadius: 12,
          ...
          {step === 'newPassword' && (
            <motion.div key="newPassword" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3 }}>
              <button
                onClick={() => { setStep('nationalId'); setError('') }}
                style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 13, color: '#6B7280', cursor: 'pointer', marginBottom: 24, background: 'none', border: 'none', padding: 0 }}
              >
                {isRTL ? <ArrowRight size={16} /> : <ArrowLeft size={16} />} {t('common.back')}
              </button>
              ...
              <div style={{ textAlign: 'center', marginBottom: 28 }}>
                ...
                <h2 style={{ fontSize: 22, fontWeight: 700, color: '#111', marginBottom: 6 }}>
                  {t('forgotPassword.setNewPassword' as any)}
                </h2>
                <p style={{ fontSize: 14, color: '#6B7280', lineHeight: 1.6 }}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div
                  style={inputWrapStyle(!!error && !newPassword)}
                  onFocus={(e) => { e.currentTarget.style.borderColor = '#002E83' }}
                  onBlur={(e) => { e.currentTarget.style.borderColor = '#E5E7EB' }}
                >
                  ...
                  <input
                    ...
                    style={inputStyle}
                  />
                  <button onClick={() => setShowNew(!showNew)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, display: 'flex' }}>
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                <div
                  style={inputWrapStyle(!!error && newPassword !== confirmPassword)}
                  onFocus={(e) => { e.currentTarget.style.borderColor = '#002E83' }}
                  onBlur={(e) => { e.currentTarget.style.borderColor = '#E5E7EB' }}
                >
                  <Lock size={18} color="#9CA3AF" />
          ...
          {step === 'otp' && (
            <motion.div key="otp" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3 }}>
              <button
                onClick={() => { setStep('newPassword'); setOtp(['', '', '', '', '', '']); setError('') }}
                style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 13, color: '#6B7280', cursor: 'pointer', marginBottom: 24, background: 'none', border: 'none', padding: 0 }}
              >
                {isRTL ? <ArrowRight size={16} /> : <ArrowLeft size={16} />} {t('common.back')}
              </button>
              ...
              <motion.button
                whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }}
                onClick={handleOtpSubmit} disabled={loading}
                style={{
                  width: '100%', height: 48, background: loading ? '#B8E67A' : '#7CFF01',
                  color: '#0F172A', fontWeight: 600, fontSize: 15, borderRadius: 12,
                  border: 'none', cursor: loading ? 'not-allowed' : 'pointer',
