...
function getStrength(pw: string): { level: number; label: string; color: string; width: string } {
  if (!pw) return { level: 0, label: '', color: '#374151', width: '0%' }
  let s = 0
  if (pw.length >= 8) s++
  if (/[A-Z]/.test(pw)) s++
  if (/[a-z]/.test(pw)) s++
...
export default function InvestorPasswordInput({ value, onChange, placeholder, error, showStrength, onSubmit }: Props) {
  const { theme, isDark } = useInvestorTheme()
  const [show, setShow] = useState(false)
  const [focused, setFocused] = useState(false)
  const strength = getStrength(value)

  const borderColor = error ? theme.red : focused ? theme.blue : isDark ? 'rgba(255,255,255,0.14)' : 'rgba(0,0,0,0.12)'
  ...
  return (
    <div>
      <div style={{ position: 'relative' }}>
        ...
        >
          {show ? <EyeOff size={18} color={theme.textTertiary} /> : <Eye size={18} color={theme.textTertiary} />}
        </button>
      </div>
      ...
      {showStrength && value && (
        <div style={{ marginTop: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <div style={{ flex: 1, height: 4, borderRadius: 2, background: isDark ? 'rgba(255,255,255,0.1)' : '#E2E8F0' }}>
              <div style={{ height: '100%', borderRadius: 2, width: strength.width, background: strength.color, transition: 'width 0.3s, background 0.3s' }} />
            </div>
            <span style={{ fontSize: 11, fontWeight: 500, color: strength.color, flexShrink: 0 }}>{strength.label}</span>
          </div>
              ...
              return (
                <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                  <div style={{
                    ...
                    background: met ? '#22C55E' : isDark ? 'rgba(255,255,255,0.1)' : '#E2E8F0',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    transition: 'background 0.2s',
                  }}>
                    {met && <Check size={8} color="#fff" strokeWidth={3} />}
                  </div>
                  <span style={{ fontSize: 11, color: met ? '#22C55E' : theme.textTertiary }}>{label}</span>
                </div>
              )
            })}
          </div>
